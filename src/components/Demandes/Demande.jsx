...
<<<<<<< HEAD
    ...
    const headerElement = (
        <div className="inline-flex align-items-center justify-content-center gap-2">
            <span className="font-bold white-space-nowrap">Modification du profile</span>
        </div>
    );
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                const formattedDemandes = employe.demmandeCollection.map(demande => ({
                    ...demande,
                    dateDemmande: new Date(demande.dateDemmande).toLocaleDateString('fr-FR')
                }));
  ...
  const fetchDemandes = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/employes/${id}`
      );
      if (response.status === 200) {
        const employe = response.data;
        const formattedDemandes = employe.demmandeCollection.map((demande) => ({
...
>>>>>>> aeeb92e1159e3fa44d41751bb1b22f7d1ea25009
...
<<<<<<< HEAD
    ...
    const deleteButton = (rowData) => {
        return <Button type="button" className="p-button-danger" onClick={() => deleteDemmande(rowData.id)} icon="pi pi-trash" />;
    };
=======
  const deleteDemmande = async (idDemmande) => {
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/demmandes/delete/${idDemmande}`
      );
      if (response.status === 204) {
...
<<<<<<< HEAD
        <div>
            ...
            <div className="m-11 mb-0 card">
                <DataTable value={demandes} paginator rows={5} rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: '50rem' }}>
                    <Column field="typeDemmande" header="Type de demande" style={{ width: '25%' }}></Column>
                    <Column field="dateDemmande" header="Date de demande" style={{ width: '25%' }}></Column>
