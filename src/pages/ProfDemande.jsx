...
const ProfDemande = () => {
    ...
    const fetchDemandes = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/api/v1/demmandes/${id}`);
    ...
    const handleAddDemmande = async () => {
        ...
        try {
            const response = await axios.post(`http://localhost:8080/api/v1/demmandes/${id}`, formData, {
                headers: {
    ...
    const headerElement = (
        <div className="inline-flex align-items-center justify-content-center gap-2">
            <span className="font-bold white-space-nowrap">Modification du profil</span>
        </div>
    );
    ...
    const deleteButton = (rowData) => {
        return <Button type="button" className="p-button-danger" onClick={() => deleteDemmande(rowData.doti)} icon="pi pi-trash" />;
    };

    const filteredDemandes = demandes.filter(demande =>
        ['Demande1', 'Demande2', 'Demande3', 'Demande4'].includes(demande.typeDemmande)
    );
        ...
        <div>
            <>
                <div className='flex justify-between items-center'>
                    ...
                    <Button className="text-white px-3 py-[.75rem] transition duration-200 ease-in-out" onClick={() => setVisible(true)}>
                        Ajouter une Demande
                    </Button>
                ...
                <div className="m-11 mb-0 card">
                    <DataTable value={filteredDemandes} paginator rows={5} rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: '50rem' }}>
                        <Column field="typeDemmande" header="Type de demande" style={{ width: '25%' }}></Column>
                        <Column field="dateDemmande" header="Date de demande" body={dateTemplate} style={{ width: '25%' }}></Column>
                        <Column field="status" header="Statut" style={{ width: '25%' }}></Column>
