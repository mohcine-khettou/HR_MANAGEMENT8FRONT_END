...
const RhDemmande = () => {
  ...
  const fetchDemandes = () => {
    axios.get('http://localhost:8080/api/v1/demmandes')
      .then(response => {
  ...
  const valeurBodyTemplate = (rowData) => {
    return rowData.typeDemmande === "EtatCivil" ? rowData.etatCivil : rowData.nmbreEnfts;
  };
  ...
  const handleAccept = (idDemmande) => {
    axios.post(`http://localhost:8080/api/v1/demmandes/accepterDemmandeUpdateProfesseur/${idDemmande}`)
      .then(response => {
        fetchDemandes(); 
      })
  ...
  const handleSendJustification = () => {
    axios.post(`http://localhost:8080/api/v1/demmandes/refuseDemmandeUpdateProfesseur/${selectedDemmande}`, justification, {
      headers: {
  ...
  const actionBodyTemplate = (rowData) => {
    if (rowData.status === 'ACCEPTE') {
      return <span className="p-2 bg-green-200 text-green-800 rounded">Acceptée</span>;
    }
    if (rowData.status === 'REFUSE') {
      return <span className="p-2 bg-red-200 text-red-800 rounded">Refusée</span>;
    }
    return (
  ...
  const footerContent = (
    <div>
        <Button label="Envoyer" icon="pi pi-check" onClick={handleSendJustification} autoFocus />
    </div>
  );

  const demandesSpecifiques = demandes.filter(demmande => 
    ["Demande1", "Demande2", "Demande3", "Demande4"].includes(demmande.typeDemmande)
  );

  const autresDemandes = demandes.filter(demmande => 
    !["Demande1", "Demande2", "Demande3", "Demande4"].includes(demmande.typeDemmande)
  );
  ...
  return (
    <div>
      ...
      <div className='flex justify-between items-center'>
          <h1 className='text-[2.35rem] font-semibold'>Demandes de modification du profile</h1>
      </div>
