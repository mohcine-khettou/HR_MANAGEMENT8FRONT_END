  ...
  const handleAddDemmande = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/api/demandes/add",
        newDemmande
      );
  ...
  const deleteDemmande = async (idDemmande) => {
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/demandes/delete/${idDemmande}`
      );
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        ...
        <Button
          className="text-white px-3 py-[.75rem] transition duration-200 ease-in-out mr-5"
